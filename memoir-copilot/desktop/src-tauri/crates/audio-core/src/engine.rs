...
/// Offline FLAC pipeline check (no microphone). Writes 60s chunks + recovery_manifest.
pub fn encode_sine_session(
    session_dir: &Path,
    seconds: u64,
    sample_rate: u32,
) -> Result<RecoveryManifest, AudioError> {
    fs::create_dir_all(session_dir)?;
    let total_samples = (seconds as u64) * (sample_rate as u64);
    let mut samples = Vec::with_capacity(total_samples as usize);
    let freq = 440.0_f32;
    let amp = 0.2_f32;
    for i in 0..total_samples {
        let t = i as f32 / sample_rate as f32;
        samples.push(amp * (2.0 * std::f32::consts::PI * freq * t).sin());
    }
    let chunk_samples = sample_rate as usize * 60;
    ...
    while offset < samples.len() {
        ...
        let dur_ms = (slice.len() as u64) * 1000 / sample_rate as u64;
        let end_ms = start_ms + dur_ms;
        let m = finalize_chunk(session_dir, idx, start_ms, end_ms, sample_rate, slice)?;
        chunks.push(m);
        idx += 1;
        start_ms = end_ms;
        offset = end;
    ...
    let manifest = RecoveryManifest {
        session_dir: session_dir.to_string_lossy().into(),
        track_id: "primary".into(),
        sample_rate,
        chunks: chunks.clone(),
        last_flush_ms: seconds * 1000,
        active_recording_ms: seconds * 1000,
        crashed_part: None,
    };
    fs::write(
        session_dir.join("recovery_manifest.json"),
